/**
 * Supply Chain Choreography — Ethereum L1 Gas Measurement
 *
 * Uses supply_chain_proofs.json generated by e2e/supply_chain_e2e.py.
 * Measures gas for instantiate, transition, and terminate operations
 * with the real Groth16 verifiers compiled for standard EVM.
 *
 * Captured metrics:
 *   - Receipt-based gasUsed, effectiveGasPrice, fee paid (wei) per operation
 *   - Call data size (bytes) per operation
 *   - Deployed bytecode size (bytes) per contract
 *   - Proof size (bytes) and public inputs count per proof type
 *   - Chain ID
 *   - Deployment tx count
 *
 * Run:
 *   REPORT_GAS=true npx hardhat test test/InstanceManagerSupplyChain.ts
 */
...
describe('InstanceManager (Supply Chain — L1)', function () {
  ...
  it('instantiation (real verifier)', async function () {
    // Deploy manually (not via Ignition) so we can capture deployment gas receipts
