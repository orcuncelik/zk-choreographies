/**
 * Supply Chain Choreography — zkSync ERA Gas Measurement
 *
 * Uses supply_chain_proofs.json generated by e2e/supply_chain_e2e.py.
 * Requires anvil-zksync running on port 8011.
 *
 * Run:
 *   # Terminal 1: start node
 *   ~/.cache/hardhat-nodejs/zksync-memory-node/0.6.11 --port=8011
 *
 *   # Terminal 2: compile and test
 *   PATH="/opt/homebrew/Cellar/node/23.10.0_1/bin:$PATH" \
 *     npx hardhat compile --network zkSyncLocalNode
 *   PATH="/opt/homebrew/Cellar/node/23.10.0_1/bin:$PATH" \
 *     npx hardhat test test/InstanceManagerZkSyncSupplyChain.ts --network zkSyncLocalNode
 */
...
// anvil-zksync pre-funded rich wallet (constant across all versions)
...
describe('InstanceManager (Supply Chain — zkSync ERA)', function () {
  ...
  before(async function () {
    ...
    // Proof size is constant: Groth16 BN254 = 8 field elements × 32 bytes each
    proofSizeBytes = proofs[0].value.length * 32;
    // Public inputs count per operation type (from proof JSON)
    ...
    // Deploy real verifiers + InstanceManager
    ...
    // Pre-instantiate states needed by transition and termination tests.
  ...
  it('instantiation (real verifier)', async function () {
    ...
    } catch { /* pubdata API not available on this node */ }
  ...
  after(function () {
    ...
    // ── Per-operation receipts (authoritative) ────────────────────────────────
    ...
    // Machine-parseable summary lines for run_choreography.sh
